...
$builtInWhere = isset($_SESSION['workflow4pool']) && count($_SESSION['workflow4pool']) > 0 ? ' statusCode = 30 AND id NOT IN (' . implode(',', $_SESSION['workflow4pool']) . ') ' : ' statusCode = 30 ';
	...
	<div class="modal-dialog modal-lg" role="document" style="width:90%;">
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<form action="admin.php?page=wf4_request" method="POST" style="display:inline-block;">
...
<div class="alert alert-info" role="alert" style="margin:10px 20px 0px 0px;">
	<h2 style="margin:0 auto;"><?php _e('Workflow Four', 'tomo'); ?></h2>
</div>
	...
	<div class="row">
		<div class="col-md-3" style="text-align:left;">
			...
			<a id="btn-showCart" class="btn btn-primary" href="javascript:void(0);">
				...
				?>
					<span class="badge"><?php echo count($_SESSION['workflow4pool']); ?></span>
				<?php
					}
				?>
			</a>
...
jQuery(document).ready(function() {
	...
	jQuery('#btn-add').attr('disabled', 'disabled').click(function(){
		addToCart(checkIds.join(), 'workflow4pool', function(){
			location = location;
		});
	});
	...
	jQuery('#maintable tbody td:not(.order_checkbox_cell)').dblclick(function(event){
		...
		addToCart(theId, 'workflow4pool', function(){
	...
	jQuery('#newProductModal').on('dblclick', '#secondaryTable tr td', function(){
		...
		removeFromCart(jQuery(this).closest('tr').attr('data-id'), 'workflow4pool', function(){
			line.fadeOut(1000);
		});
	});
	
	jQuery('#secondaryRemove').click(function(){
		removeFromCart(checkSecondaryIds.join(), 'workflow4pool', function(){
