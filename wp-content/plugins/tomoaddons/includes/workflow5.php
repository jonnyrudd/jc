...
$builtInWhere = isset($_SESSION['workflow5pool']) && count($_SESSION['workflow5pool']) > 0 ? ' statusCode = 40 AND id NOT IN (' . implode(',', $_SESSION['workflow5pool']) . ') ' : ' statusCode = 40 ';
	...
	<div class="modal-dialog modal-lg" role="document" style="width:90%;">
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<form action="admin.php?page=wf5_request" method="POST" style="display:inline-block;">
...
<div class="alert alert-info" role="alert" style="margin:10px 20px 0px 0px;">
	<h2 style="margin:0 auto;"><?php _e('Workflow Five', 'tomo'); ?></h2>
</div>
	...
	<div class="row">
		<div class="col-md-3" style="text-align:left;">
			...
			<a id="btn-showCart" class="btn btn-primary" href="javascript:void(0);">
				...
				?>
					<span class="badge"><?php echo count($_SESSION['workflow5pool']); ?></span>
				<?php
					}
				?>
			</a>
...
jQuery(document).ready(function() {
	...
	jQuery('#btn-add').attr('disabled', 'disabled').click(function(){
		addToCart(checkIds.join(), 'workflow5pool', function(){
			location = location;
		});
	});
	...
	jQuery('#maintable tbody td:not(.order_checkbox_cell)').dblclick(function(event){
		...
		addToCart(theId, 'workflow5pool', function(){
	...
	jQuery('#newProductModal').on('dblclick', '#secondaryTable tr td', function(){
		...
		removeFromCart(jQuery(this).closest('tr').attr('data-id'), 'workflow5pool', function(){
			line.fadeOut(1000);
		});
	});
	
	jQuery('#secondaryRemove').click(function(){
		removeFromCart(checkSecondaryIds.join(), 'workflow5pool', function(){
