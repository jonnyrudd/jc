...
<div class="alert alert-info" role="alert" style="margin:10px 20px 0px 0px;">
	<h2 style="margin:0 auto;"><?php _e('Client Maintenance', 'tomo'); ?></h2>
</div>
<div style="padding:10px 20px 0px 0px;">
	...
	<div class="row">
		...
		<div class="col-md-2" style="text-align:right;">
			<a id="btn-add" class="btn btn-success" href="admin.php?page=client_request"><i class="fas fa-plus-circle"></i> <?php _e('Add New', 'tomo'); ?></a>
		</div>
	</div>
	
</div>
<?php
		$prodFields = $wpdb->get_results('SHOW FULL COLUMNS FROM vw_active_client', ARRAY_N);
		$prodCols = array();
?>
<div style="margin:5px 20px 0px 0px; overflow-x:auto;">
	<div style="min-width:100%; width:<?php echo count($prodFields) * 150; ?>px;margin-bottom:0px;">
...
jQuery(document).ready(function() {
	...
	jQuery('#btn-reset').on('click', function () {
		location = 'admin.php?page=client_info';
	});
		...
		jQuery.ajax({
			...
			success: function(data) {
				if(data == 'error'){
					...
					var innerWrapper = jQuery('<div style="min-width:100%; width:<?php echo count($prodFields) * 150; ?>px;margin-bottom:0px;"></div>');
