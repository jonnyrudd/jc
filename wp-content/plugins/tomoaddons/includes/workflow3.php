...
$builtInWhere = isset($_SESSION['workflow3pool']) && count($_SESSION['workflow3pool']) > 0 ? ' statusCode = 20 AND id NOT IN (' . implode(',', $_SESSION['workflow3pool']) . ') ' : ' statusCode = 20 ';
	...
	<div class="modal-dialog modal-lg" role="document" style="width:90%;">
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<form action="admin.php?page=wf3_request" method="POST" style="display:inline-block;">
					<select class="form-control" name="warehouse" style="margin-bottom:10px;">
						<?php
						foreach($wpdb->get_results('SELECT id, name FROM vw_active_warehouse', ARRAY_A) AS $line){
							$wid 	= $line['id'];
							$wname 	= $line['name'];
					...
					<input id="startWorkflow" type="submit" class="btn btn-primary" value="<?php _e('Start Workflow', 'tomo'); ?>" />
				</form>
			</div>
		</div>
	</div>
...
<div class="alert alert-info" role="alert" style="margin:10px 20px 0px 0px;">
	<h2 style="margin:0 auto;"><?php _e('Workflow Three', 'tomo'); ?></h2>
</div>
	...
	<div class="row">
		<div class="col-md-3" style="text-align:left;">
			...
			<a id="btn-showCart" class="btn btn-primary" href="javascript:void(0);">
				...
				?>
					<span class="badge"><?php echo count($_SESSION['workflow3pool']); ?></span>
				<?php
					}
				?>
			</a>
...
jQuery(document).ready(function() {
	...
	jQuery('#btn-add').attr('disabled', 'disabled').click(function(){
		addToCart(checkIds.join(), 'workflow3pool', function(){
			location = location;
		});
	});
	...
	jQuery('#maintable tbody td:not(.order_checkbox_cell)').dblclick(function(event){
		...
		addToCart(theId, 'workflow3pool', function(){
	...
	jQuery('#newProductModal').on('dblclick', '#secondaryTable tr td', function(){
		...
		removeFromCart(jQuery(this).closest('tr').attr('data-id'), 'workflow3pool', function(){
			line.fadeOut(1000);
		});
	});
	
	jQuery('#secondaryRemove').click(function(){
		removeFromCart(checkSecondaryIds.join(), 'workflow3pool', function(){
