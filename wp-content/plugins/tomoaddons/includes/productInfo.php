...
<div class="alert alert-info" role="alert" style="margin:10px 20px 0px 0px;">
	<h2 style="margin:0 auto;"><?php _e('Product Maintenance', 'tomo'); ?></h2>
</div>
<div style="padding:10px 20px 0px 0px;">
	...
	<div class="row">
		...
		<div class="col-md-2" style="text-align:right;">
			<a id="btn-add" class="btn btn-success" href="admin.php?page=product_request"><i class="fas fa-plus-circle"></i> <?php _e('Add New', 'tomo'); ?></a>
		</div>
	</div>
	
</div>
<div style="margin:5px 20px 0px 0px; overflow-x:scroll;">
	<div style="width:2500px;margin-bottom:0px;">
		<table id="maintable" class="table table-striped table-bordered table-hover table-sm" style="font-size:12px;margin-bottom:0px;">
			<thead class="thead-dark" style="font-weight:bold;">
				<tr>
<?php
		$prodFields = $wpdb->get_results('SHOW FULL COLUMNS FROM vw_active_product', ARRAY_N);
		$prodCols = array();
...
jQuery(document).ready(function() {
	...
	jQuery('#btn-reset').on('click', function () {
		location = 'admin.php?page=product_info';
	});
		...
		jQuery.ajax({
			...
			success: function(data) {
				if(data == 'error'){
					...
					var wrapper = jQuery('<div style="margin:5px 20px 0px 0px; overflow-x:scroll;"></div>');
					var innerWrapper = jQuery('<div style="width:2500px;margin-bottom:0px;"></div>');
